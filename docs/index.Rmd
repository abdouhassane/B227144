...
```{r tidy the heart disease dataset and  plot on the map to show changes over time}
#focusing the dataset on emergency admissions with a focus a crude rate. crude rate is defined as the number of events ocurring during a specificed time multiplied by 100,000 of the population.
...
heart_disease_map_plot <- heart_disease_map %>% 
  ggplot(aes(geometry = geometry, fill = crude_rate))+
  geom_sf()

change <- heart_disease_map_plot + 
      labs(title = "Emergency Admissions for heart disease over from 2019 to 2023 {as_integer(frame_time)}") + transition_time(financial_year)

animate_change <- animate(change, fps = 5)
...
clean_statin4years_plot <- statin4years %>% 
  full_join(population_2020_2023, by = c("HBName", "paid_date_month"="year")) %>% 
  filter(!is.na(population)) %>% 
  select(-hb) %>% 
  mutate(quantity_per_head = (Total_paid_quantity/mean(population))) %>%
...
clean_statin4years_plot/wrap_table(clean_statin4years_table, space = "free_y")
